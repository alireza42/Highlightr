...
var Skimmy = function(options) {
    ...
    this.options = extend({}, Skimmy.defaultOptions, options);
...
Skimmy.version = "25.04.2013-09:55:11"; // filled automatically by hook
Skimmy.LocationHandler = LocationHandler;

Skimmy.defaultOptions = {
    'regexp': "[^\\s,;:\u2013.!?<>\u2026\\n\u00a0\\*]+",
...
Skimmy.prototype = {
    ...
    destroy: function(){
        ...
        var indices = byClassName(this.selectable, 'skimmy_index');
    ...
    markerClick: function(e){
        ...
        if (hasClass(this.marker, 'skimmy-marker-bar')){
            if (!hasClass(target, 'skimmy-social') && !hasClass(target, 'skimmy-marker')){
                return;
            }
        }
        removeClass(this.marker, 'show');
        ...
        if (hasClass(target, 'skimmy-social') ){
    ...
    checkSentence: function(range){
        ...
        if (nextAfterRange.match(/\.|\?|\!/)){
            ...
            if (range.startOffset == 0 &&
                ...
                hasClass(range.startContainer.previousSibling, 'skimmy_index')){
                return apply();
            }

            var node, iterator = range.getElementIterator();
            while ((node=iterator())) {
                if (node.nodeType == 1 && hasClass(node, 'skimmy_index')){
                    return apply();
                }
            }
            ...
            for (var idx=0; idx<children.length; ++idx) {
                ...
                if (nodeType==3 && !child.nodeValue.match(this_.regexp)) {
                    ...
                    if (!blockStarted){
                        ...
                        // XXX prefix all class and id attributes with "skimmy"
                        index_span.className = 'skimmy_index skimmy_index' + this_.captureCount;
    ...
    isFirstTextNode: function(textNode){
        ...
        for (var i=prevs.length;i--;){
            if (prevs[i] && prevs[i].nodeType == 1 && prevs[i].className == 'skimmy_index'){
                return true;
            }
        }
        return false;
    ...
    getFirstTextNode: function(numclass){
        ...
        var tnode = byClassName(this.selectable, 'skimmy_index'+numclass)[0];
        if (tnode) {
    ...
    getNum: function(cont){
        while (cont.parentNode){
            while (cont.previousSibling){
                ...
                if (cont.nodeType == 1 && hasClass(cont, 'skimmy_index')){
...
,
    ...
    getMessageClosed: function(){
        if (window.localStorage){
            return !!localStorage.skimmy_warning;
        } else {
            return !!document.cookie.match(/(?:^|;)\s*skimmy-warning=/);
        }
    },
    saveMessageClosed: function(){
        if (window.localStorage){
            localStorage.skimmy_warning = 'true';
        } else {
            // XXX need to be tested under IE
            if (!this.getMessageClosed()){
                document.cookie += '; skimmy-warning=true';
            }
        }
    },
...
MultiLocationHandler.prototype = {
    ...
    addHashchange: Skimmy.LocationHandler.prototype.addHashchange,
...
var MultiSkimmy = function(elements, getPrefix, options){
...
window.Skimmy = Skimmy;

if (window.jQuery){
    window.jQuery.fn.skimmy = function(options) {
        options = options || {};
        options = extend({'selectable': this[0]}, options);
        return new Skimmy(options);
    };
}
window.MultiSkimmy = MultiSkimmy;
